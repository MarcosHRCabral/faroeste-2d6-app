...
export function useSocketMultiplayerSession(initialCode = "", enabled = false) {
  const { socket, socketConfigured, socketUrl, socketConfig, connectionError, status } =
    useSocketConnection(enabled);
  ...
  const [credentials, setCredentials] = useState<SessionCredentials | null>(() =>
    enabled ? loadStoredCredentials(initialCode || undefined) : null
  );
  const [error, setError] = useState("");

  useEffect(() => {
    if (enabled && !credentials) {
      setCredentials(loadStoredCredentials(initialCode || undefined));
    }
  }, [credentials, enabled, initialCode]);
  ...
  useEffect(() => {
    if (!socket || !credentials || status !== "connected" || !enabled) {
      return;
    }
  ...
  }, [credentials, enabled, socket, status]);
